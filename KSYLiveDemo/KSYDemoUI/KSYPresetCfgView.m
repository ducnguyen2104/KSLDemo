...
- (id) init {
    ...
    _lblStreamResoUI = [self addLable:@"Push stream resolution"];
    ...
    _frameRateUI  = [self addSliderName:@"Video frame rate fps" From:1.0 To:30.0 Init:15.0];
    _lblVideoCodecUI = [self addLable:@"Video encoder"];
    _videoCodecUI = [self addSegCtrlWithItems:@[@"automatic",@"soft264",@"hard264",@"soft265", @"hard265"]];
    ...
    _lblAudioKbpsUI= [self addLable:@"Audio bit rate kbps"];
    ...
    _bwEstModeUI = [self addSegCtrlWithItems:@[@"default", @"Smooth", @"closure"]];
    _lblWithMessage  = [self addLable:@"Message channel"];
    _withMessageUI = [self addSegCtrlWithItems:@[@"Open", @"closure"]];
    _demoLable    = [self addLable:@"Select demo to start"];
...
- (void) selectProfile:(NSInteger)idx {
    ...
    if (idx == 0){
        ...
        if (_bRecord){
            ...
            [_btn0 setTitle: @"Live broadcast with preset configuration" forState: UIControlStateNormal];
        }
    }else{
        ...
        if (_bRecord){
            ...
            [_btn0 setTitle: @"Custom configuration live broadcast" forState: UIControlStateNormal];
        }
    }
    [self layoutUI];
}
