...
- (void) defaultStramCfg{
    ...
    // Set the encoding scene
    _kit.streamerBase.liveScene       = KSYLiveScene_Default;
    // Set encoding rate control
    _kit.streamerBase.recScene        = KSYRecScene_ConstantQuality;
    // Video coding performance grade (HighPerformance is recommended for hard coding)
    ...
    else { // Recommended for soft coding lowpower
...
- (void) updateStreamCfg: (BOOL) bStart {
    ...
    _miscView.swBypassRec.enabled = !_bRecord; // When recording directly, you cannot bypass recording
    if (_bRecord && bStart){
...
- (void) updateSwAudioOnly : (BOOL) bStart {
    if (bStart) {
        if (self.audioView.swAudioOnly.on) {  // Pure audio streaming is enabled
            self.audioView.swAudioOnly.enabled = NO;
            self.audioView.lblAudioOnly.text =@"Pure audio streaming";
            _kit.streamerBase.bWithVideo = NO;  // Close video
        }
        else {
            self.audioView.swAudioOnly.enabled = YES;
            self.audioView.lblAudioOnly.text =@"Freeze screen";
            _kit.streamerBase.bWithVideo = YES; //Pure audio streaming is not enabled, video is enabled
        }
    }
    else {
...
- (void) onNetStateEvent     :(NSNotification *)notification{
    //Record the number of occurrences of events such as network congestion
...
- (void) onStreamStateChange :(NSNotification *)notification{
    ...
    if(_kit.streamerBase.streamState == KSYStreamStateError) {
        ...
        [_ctrlView.lblStat initStreamStat]; // Reset statistics when trying to start connection
...
- (void)onTimer:(NSTimer *)theTimer{
    ...
    _strSeconds++;
    [self updateLogoText];
    [self updateRecLabel];  // Local recording: store directly to the local, without streaming
    [self updateBypassRecLable];////Bypass recording: video while streaming
}
...
- (void)onMenuBtnPress:(UIButton *)btn{
    ...
    if (btn == _ctrlView.menuBtns[0] ){
        view = _ksyBgmView; // Background music playback related
    }
    else if (btn == _ctrlView.menuBtns[1] ){
        view = _ksyFilterView; // Beauty filter related
    }
    else if (btn == _ctrlView.menuBtns[2] ){
        view = _audioView;    // Mixing console
    ...
    // Hiding the menu button will trigger the view display of the secondary menu
    if (view){
...
- (void)onBgmCtrSle:(UISegmentedControl*)sender {
    if ( sender == _ksyBgmView.loopType){
        weakObj(self);
        if ( sender.selectedSegmentIndex == 0) { //Single play
...
- (void)onBgmSlider:(id )sl{
    if (sl == _ksyBgmView.volumSl){
        ...
        // Modify the local and audience pitch at the same time (the recommended range of pitch shift is an integer from -3 to 3)
...
- (void) onCapture{
    if (!_kit.vCapDev.isRunning){
        ...
        _kit.videoOrientation = [[UIApplication sharedApplication] statusBarOrientation];
        // To reopen the preview, you need to re-setupLogo according to the direction
        [self setupLogo];
        [_kit startPreview:_bgView];
    }
    else {
...
- (void)onAMixerSwitch:(UISwitch *)sw{
    if (sw == _audioView.muteStream){
        ...
        if ( ![KSYAUAudioCapture isHeadsetPluggedIn] ) {
            [KSYUIVC toast:@"Without headphones, there will be a harsh sound when the ear return is turned on" time:0.3];
            sw.on = NO;
        ...
        if (_audioView.audioEffect != KSYAudioEffectType_COUSTOM){
            [KSYUIVC toast:@"Switch to custom mode to open" time:0.3];
            sw.on = NO;
            return;
        }
        if (sw.on){
...
-(void) onBypassRecord {
    ...
    if (_miscView.swBypassRec.on){
        if ( _kit.streamerBase.isStreaming && !bRec){
            ...
            NSString * msg = @"The video can be bypassed during the streaming process";
...
-(void) updateBypassRecLable {
    ...
    if (dur > REC_MAX_TIME) { // In order to prevent the phone storage from being full, limit the bypass recording time to 30s
        _miscView.swBypassRec.on = NO;
        [_kit.streamerBase stopBypassRecord];
    }
}
...
- (void) updateRecLabel {
    ...
    if (_kit.streamerBase.isStreaming && diff < 0){
        [self onStream];//End recording
    }
    if (_kit.streamerBase.isStreaming){//Countdown time during recording
...
//Delete the file to ensure that the video time saved in the album is the latest
...
- (void)pinchDetected:(UIPinchGestureRecognizer *)recognizer{
    ...
    CGFloat zoomFactor = _currentPinchZoomFactor * recognizer.scale;//Current touch zoom factor * coordinate scale
