...
- (void)viewDidLoad {
    ...
    //推流地址
    NSString *streamSrv  = @"rtmp://192.168.1.122/live";
...
- (void)initVariable{
    ...
    [self addMenu:@"Push streaming demo"     withBlk:^(NSURL* url){return [[KSYPresetCfgVC alloc]initWithURL:url];} ];
    [self addMenu:@"Minimal push streaming"      withBlk:^(NSURL* url){return [[KSYSimplestStreamerVC alloc] initWithUrl:url];} ];
    [self addMenu:@"Half screen push"      withBlk:^(NSURL* url){return [[KSYHorScreenStreamerVC alloc] initWithUrl:url];} ];
    [self addMenu:@"Brush Push Stream"      withBlk:^(NSURL* url){return [[KSYBrushStreamerVC alloc] initWithUrl:url];} ];
    [self addMenu:@"Background image push"   withBlk:^(NSURL* url){return [[KSYBgpStreamerVC alloc] initWithUrl:url];} ];
    [self addMenu:@"Record push stream short video" withBlk:^(NSURL* url){
